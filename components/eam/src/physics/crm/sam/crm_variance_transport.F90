...
subroutine CVT_diagnose_mom(ncrms)
   ...
   ! local variables
   integer, parameter :: lensav = nx+15 ! must be at least N + INT(LOG(REAL(N))) + 4.
   real(crm_rknd), dimension(nx)    :: fft_out_u   ! for FFT input and output
   real(crm_rknd), dimension(nx)    :: work        ! work array
   real(crm_rknd), dimension(lensav):: wsave       ! prime factors of N and certain trig values used in rfft1f
   integer :: i, j, k, icrm   ! loop iterators
   integer :: ier             ! error return code
...
subroutine CVT_forcing_mom(ncrms)
   ...
   ! local variables
   real(crm_rknd), allocatable :: utend_loc(:,:,:,:)
   real(crm_rknd) :: angle,freq
   integer :: i, j, k, icrm, iwn   ! loop iterators
   !----------------------------------------------------------------------------
   ...
   do k = 1,nzm
      do j = 1,ny
         do i = 1,nx
            do icrm = 1,ncrms
               ! u(icrm,i,j,k) = u(icrm,i,j,k) + utend_loc(icrm,i,j,k) * dtn
               dudt(icrm,i,j,k,na) = dudt(icrm,i,j,k,na) + utend_loc(icrm,i,j,k)! * dtn
            end do
         end do
      end do
   end do
