...
#if defined(MMF_SCVT) || defined(MMF_BCVT)
      do iwn = 1,crm_nvark
        t_cvt_tend(icrm,k,iwn) = ( crm_input%t_cvt(icrm,l,iwn) - t_cvt(icrm,k,iwn) )*idt_gl
        q_cvt_tend(icrm,k,iwn) = ( crm_input%q_cvt(icrm,l,iwn) - q_cvt(icrm,k,iwn) )*idt_gl
#if defined(MMF_SCVT_MOM) || defined(MMF_BCVT_MOM)
        u_cvt_tend(icrm,k,iwn) = ( crm_input%u_cvt(icrm,l,iwn) - u_cvt(icrm,k,iwn) )*idt_gl
#endif
      end do
#endif
    end do ! k
  ...
  do iwn = 1,crm_nvark
    do k = 1,plev
      do icrm = 1,ncrms
        if ( k>(ptop+1) ) then
          l = plev-k+1
          crm_output%t_cvt_tend(icrm,k,iwn) = ( t_cvt(icrm,l,iwn) - crm_input%t_cvt(icrm,k,iwn) ) * icrm_run_time
          crm_output%q_cvt_tend(icrm,k,iwn) = ( q_cvt(icrm,l,iwn) - crm_input%q_cvt(icrm,k,iwn) ) * icrm_run_time
#if defined(MMF_SCVT_MOM) || defined(MMF_BCVT_MOM)
          crm_output%u_cvt_tend(icrm,k,iwn) = ( u_cvt(icrm,l,iwn) - crm_input%u_cvt(icrm,k,iwn) ) * icrm_run_time
#endif
        else
          crm_output%t_cvt_tend(icrm,k,iwn) = 0.
          crm_output%q_cvt_tend(icrm,k,iwn) = 0.
          crm_output%u_cvt_tend(icrm,k,iwn) = 0.
        end if
      end do
    end do
  end do
